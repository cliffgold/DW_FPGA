...
`include "structs.svh"
   ...
   reg signed [MAX_SUM-1:0]      rnd_bits;

   reg signed [MAX_SUM:0] 	 old_sum [0:MAX_RUN_BITS];
   reg signed [MAX_SUM+2:0] 	 old_sum_j;

   reg signed [MAX_SUM:0] 	 full_sum_q;
   reg signed [MAX_SUM:0] 	 full_sum_q1;
   reg signed [MAX_SUM:0] 	 full_sum_q2;
   ...
   always@(posedge sys.clk ) begin
	 ...
	 if (pcie_pick_req.vld) begin
	    pick_pcie_rd.data <= {{64-MAX_RUNS-1-MAX_SUM-1{1'b0}},
				  run_q[MAX_RUNS:0],
				  old_sum[pcie_pick_req.addr[MAX_SUM:0]]};
